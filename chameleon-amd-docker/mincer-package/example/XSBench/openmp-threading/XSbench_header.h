...
typedef struct {
    ...
    int num_events;
    int event_codes[10];
    long long values_before[10];
    long long values_after[10];
    char event_names[10][256];
    int initialized;
} EnergyMonitor;

// Energy monitoring function declarations
int init_energy_monitoring(EnergyMonitor *monitor);
void start_energy_measurement(EnergyMonitor *monitor);
void stop_energy_measurement(EnergyMonitor *monitor);
void print_energy_results(EnergyMonitor *monitor, double runtime);
void cleanup_energy_monitoring(EnergyMonitor *monitor);
...
// energy_perf_monitor.c
#ifdef PAPI
int init_rapl_monitoring(void);
void start_rapl_measurement(void);
void stop_rapl_measurement(void);
void print_rapl_results(double runtime);
void cleanup_rapl_monitoring(void);
#endif
