...
int init_rapl_monitoring() {
    ...
    if (retval == PAPI_OK) {
        ...
        if (PAPI_add_event(EventSet_PERF, PAPI_TOT_CYC) == PAPI_OK) {
            perf_events_added++;
        }
        
        if (PAPI_add_event(EventSet_PERF, PAPI_TOT_INS) == PAPI_OK) {
            perf_events_added++;
        }
        
        if (perf_events_added == 2) {  // Both cycles and instructions
            perf_initialized = 1;
            printf("Energy + Performance monitoring initialized successfully\n");
        } else {
            printf("Energy monitoring initialized (performance counters unavailable)\n");
        }
    } else {
...
void start_rapl_measurement() {
    ...
    printf("Energy + Performance measurement started\n");
}
...
void stop_rapl_measurement() {
    ...
    printf("Energy + Performance measurement stopped\n");
}
...
void cleanup_rapl_monitoring() {
    ...
    PAPI_shutdown();
    printf("Energy + Performance monitoring cleaned up\n");
}
