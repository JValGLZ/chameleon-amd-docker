...
int main( int argc, char* argv[] )
{
	...
	// Initialize RAPL monitoring
	#ifdef PAPI
	if (mype == 0) {
		if (init_rapl_monitoring() != 0) {
			printf("Warning: RAPL monitoring initialization failed\n");
		}
	}
	#endif
	...
	// Stop RAPL measurement and print results
	#ifdef PAPI
	if (mype == 0) {
		stop_rapl_measurement();
		print_rapl_results(omp_end - omp_start);
	}
	#endif
	...
	// Cleanup RAPL monitoring
	#ifdef PAPI
	if (mype == 0) {
		cleanup_rapl_monitoring();
	}
	#endif
